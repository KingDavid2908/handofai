...
async function scriptInjectedFrames(tabId: number): Promise<Map<number, boolean>> {
  try {
    ...
    // Silently ignore errors from error pages (chrome://, chrome-error://, etc.)
    // These are expected when the tab is not a regular web page
    if (err?.message?.includes('showing error page')) {
      return new Map();
    }
