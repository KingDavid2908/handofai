  ...
  switch (modelConfig.provider) {
    ...
    case ProviderTypeEnum.OpenRouter: {
      ...
      return createOpenAIChatModel(providerConfig, modelConfig, {
        headers: {
          'HTTP-Referer': 'https://handofai.local',
          'X-Title': 'HandofAI',
        },
      });
    }
...
// Creates a proxied chat model that routes LLM calls through the handofai WebSocket bridge
export function createProxiedChatModel(
  ...
  const original = createChatModel(providerConfig, modelConfig);

  return new Proxy(original, {
    get(target, prop) {
      if (prop === 'invoke') {
        return async function(messages: any[], options?: any) {
          const id = String(++llmCallCounter.value);
          return new Promise((resolve, reject) => {
            pendingLlmCalls.set(id, { resolve, reject });
